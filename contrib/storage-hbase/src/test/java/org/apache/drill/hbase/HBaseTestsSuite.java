...
public class HBaseTestsSuite {
      ...
      synchronized (HBaseTestsSuite.class) {
        if (initCount.get() == 0) {
          ...
          if (manageHBaseCluster) {
            ...
            UTIL.startMiniHBaseCluster(1, 1/*?????????3*//*?? WAS: 1*/);
  ...
  private static void createTestTables() throws Exception {
     ...
     */
    TestTableGenerator.generateHBaseDatasetNoRows(admin, TEST_TABLE_NO_ROWS, 2);
    TestTableGenerator.generateHBaseDataset1(admin, TEST_TABLE_1, 2/*???WAS: 1*/);
